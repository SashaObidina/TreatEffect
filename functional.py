...
import grad  # noqa: F401
import utils
from utils import _single, _pair, _triple, _list_with_default
...
def multi_head_attention_forward(
    ...
    if need_weights:
        ...
        if attn_mask is not None:
            covariance_matrix = models.compute_inv_covariance_matrix(k.permute(0, 2, 1))
            #print(covariance_matrix)
            #attn_output_weights = torch.baddbmm(attn_mask, q_scaled, k_scaled.transpose(-2, -1))
            #print('attn_output_weights', attn_output_weights.shape)
            attn_output_weights = models.mahalanobis_attention(q_scaled, k, attn_mask, covariance_matrix)
            #print('attn_output_weights', attn_output_weights.shape)
            #print('attn_output_weights[0][0]', attn_output_weights[0][0])
        else:
            attn_output_weights = torch.bmm(q_scaled, k.transpose(-2, -1))
        #print(attn_output_weights[0][0][attn_output_weights[0][0] != 0])
        attn_output_weights = softmax(attn_output_weights, dim=-1)
        #print(attn_output_weights[0][0][attn_output_weights[0][0] != 0])
        #print(torch.sum(attn_output_weights[0][0]))
        if dropout_p > 0.0:
